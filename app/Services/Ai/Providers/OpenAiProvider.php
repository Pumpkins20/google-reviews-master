...
class OpenAiProvider implements AiProviderInterface
{
    ...
    public function analyzeBatch(array $reviews): array
    {
        ...
        if ($response->status() === 429) {
            throw new Exception("OpenAI API Rate Limit exceeded (429).");
        }

        if ($response->failed()) {
            throw new Exception("OpenAI API call failed: " . $response->status() . " - " . $response->body());
        }
        ...
        if (empty($textResponse)) {
            throw new Exception("OpenAI returned an empty text response. Raw response: " . json_encode($rawResponse));
        }

        $parsedData = json_decode($textResponse, true);

        if (!is_array($parsedData) || !isset($parsedData['reviews']) || !is_array($parsedData['reviews'])) {
            throw new Exception("OpenAI output format was invalid (expected key 'reviews'): " . $textResponse);
        }
        ...
        foreach ($reviews as $originalReview) {
            ...
            if (isset($parsedReviewsMap[$id])) {
                ...
                Log::warning("Review ID {$id} was sent to OpenAI but not returned in the structured output.");
                $results[] = [
                    'review_id' => $id,
                    'category' => 'PERTANYAAN',
                    'spam_label' => 'ham',
                    'confidence' => 0.5,
                    'reason' => 'Gagal dianalisis oleh OpenAI (ID tidak dikembalikan oleh API).',
                    'execution_time_ms' => round($apportionedTime, 2),
                    'input_tokens' => (int) round($apportionedInputTokens),
                    'output_tokens' => (int) round($apportionedOutputTokens),
                    'cost' => round($apportionedCost, 6),
